...
export default {
    ...
    methods: {
        ...
        addIngredient() {
            const defaultIngredient = this.availableIngredients.length > 0 ? this.availableIngredients[0].id : "";
        ...
        addCategory() {
            const defaultCategory = this.availableCategories.length > 0 ? this.availableCategories[0].id : "";
            ...
            try {
                const ingredientPromises = this.cocktail.ingredients.map(ing => {
                    if (ing.ingredient_id === "new") {
                        return axios.post('http://localhost:3000/ingredients', { nom: ing.newIngredientName }, {
                            headers: { Authorization: `Bearer ${token}` }
                        }).then(response => {
                ...
                const categoryPromises = this.cocktail.categories.map((cat, index) => {
                    if (cat === "new") {
                        return axios.post('http://localhost:3000/categories', { nom: this.newCategories[index] }, {
                            headers: { Authorization: `Bearer ${token}` }
                        }).then(response => {
